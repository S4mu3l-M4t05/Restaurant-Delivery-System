...
public class EndDeliveryController {

	private EndDeliveriesView view;
	private EndDeliveriesModel model;

	private List<Integer> ordersIDList;

	public EndDeliveryController(EndDeliveriesView view, EndDeliveriesModel model) {
		this.view = view;
		this.model = model;
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (selectedIndex != -1) {
					...
					if (selectedOrder != null) {
						...
						JOptionPane.showMessageDialog(view, "Failed to retrieve order details.", "Error",
								JOptionPane.ERROR_MESSAGE);
					}
				} else {
					JOptionPane.showMessageDialog(view, "Please select an item from the list.", "No item selected",
		...
		view.getCompleteButton().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (!view.getDeliveryDetailsBox().getText().equals("")) {
					int option = JOptionPane.showConfirmDialog(null, "Do you want to Take this order?", "Confirmation",
							JOptionPane.YES_NO_OPTION);
					if (option == JOptionPane.YES_OPTION) {
						int selectedIndex = view.getDeliveryList().getSelectedIndex();
						if (selectedIndex != -1) { // Check if an item is selected
							model.updateOrderStatus(ordersIDList.get(selectedIndex), "Delivered");
							JOptionPane.showMessageDialog(null, "Order has assigned to you !", "Success",
									JOptionPane.INFORMATION_MESSAGE);
							DefaultListModel<String> model = (DefaultListModel<String>) view.getDeliveryList()
									.getModel();
							...
							JOptionPane.showMessageDialog(null, "Please select an order to accept.", "Error",
									JOptionPane.ERROR_MESSAGE);
						}
					}
				} else {
